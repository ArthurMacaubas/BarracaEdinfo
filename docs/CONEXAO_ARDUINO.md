...
> Nunca conecte 127/220 V diretamente ao Arduino Uno. Para cargas ligadas à rede elétrica, use relé certificado, fusível, caixa isolante e instalação realizada por profissional habilitado. Não alimente a fita LED ou a sirene por um pino digital do Arduino.

## Gravação do firmware

Abra `firmware/arduino_barraca_agostina/arduino_barraca_agostina.ino` na Arduino IDE. Selecione **Arduino Uno** em **Ferramentas → Placa → Arduino AVR Boards**, escolha a porta USB correta e grave o sketch. O Monitor Serial deve usar **115200 bps**.

O firmware envia `READY|barraca-agostina-arduino-uno` ao iniciar e recebe uma linha por comando:
...
Primeiro grave o firmware, abra o Monitor Serial a 115200 bps e confirme a mensagem `READY|barraca-agostina-arduino-uno`. Depois feche o Monitor Serial, conecte o Arduino Uno ao computador e inicie o backend com `npm run dev`. Em **Cadastro & hardware**, use o teste dos relés e confirme que o painel mostra `ONLINE` e registra a resposta da placa.
