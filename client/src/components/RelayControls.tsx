...
export default function RelayControls() {
  ...
  const cards: Relay[] = [{ label: "Relé da fita LED", state: relays.led, detail: online ? "Controle manual da iluminação da barraca." : "Conecte o controlador para liberar o acionamento.", icon: Lightbulb, action: <Button className="w-full rounded-xl" variant={relays.led === "ON" ? "outline" : "default"} disabled={disabled} onClick={() => led.mutate({ enabled: relays.led !== "ON" })}><Power className="mr-2 h-4 w-4" />{relays.led === "ON" ? "Desligar fita" : "Ligar fita"}</Button> }, { label: "Relé da sirene", state: relays.siren, detail: online ? "Acionamento manual limitado a 1 segundo por segurança." : "Conecte o controlador para liberar o acionamento.", icon: Siren, action: <div className="grid grid-cols-2 gap-2"><Button className="rounded-xl bg-[#9c321e] hover:bg-[#7f2819]" disabled={disabled} onClick={() => siren.mutate({ enabled: true, durationMs: 1_000 })}><TimerReset className="mr-1.5 h-4 w-4" />Tocar 1 s</Button><Button className="rounded-xl" variant="outline" disabled={disabled} onClick={() => siren.mutate({ enabled: false })}>Desligar</Button></div> }];
